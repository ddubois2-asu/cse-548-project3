...
class Firewall (EventMixin):
	...
	def installFlow(self, event, offset, srcmac, dstmac, srcip, dstip, sport, dport, nwproto):
        ...
        def allow(self, event, offset, srcmac, dstmac, srcip, dstip, sport, dport, nwproto):
		msg = of.ofp_flow_mod()
		match = of.ofp_match()
		if(srcip != None):
			match.nw_src = IPAddr(srcip)
		...
		if(match.dl_type == packet.IP_TYPE):
		  ...
		  if ip_packet.protocol == ip_packet.TCP_PROTOCOL:
			log.debug("TCP it is !")
                  ...
                  #print packet.src
                  found = False
                  for (mac, ip) in self.macToIp:
                    if packet.src == mac:
                      found = True
                      #if ip_packet.srcip != ip:
                        #self.installFlow(event, 1, None, "any", "any", "any", "any", "any", "any")
                      #  self.installFlow(event, 1, mac, None, ip_packet.srcip, None, None, None, ip_packet.protocol)
                  if(not found):
                    self.allow(event, 1, packet.src, None, ip_packet.srcip, None, None, None, ip_packet.protocol)
                    self.installFlow(event, 0, packet.src, None, None, None, None, None, ip_packet.protocol)
                    self.macToIp.append((packet.src,ip_packet.srcip ))
